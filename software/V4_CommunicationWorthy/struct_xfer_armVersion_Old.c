...
#include "struct_xfer_armVersion_Old.h"

// Reads struct straight up from uart buffer
bool recv_struct( uint32_t uart, struct arm_control_struct* myStruct)
{
	// Variables
	uint8_t i;
	...
	const int size = sizeof(*((struct arm_control_struct *)myStruct));
	...
	//check size:
	temp = UARTCharGetNonBlocking(uart);
	if(size != temp)
